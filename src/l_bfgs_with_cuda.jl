...
# Fix T definition here for initial_state TODO
...
function optimize_CUDA(d::D, initial_x::Tx, method::M,
    ...
    while !converged && !stopped && iteration < options.iterations
        ...
        #TODO OnceDifferentiable_CUDA has no field h_calls. nasel sem v JuliaNLSolvers/Optim/src/api.jl, ze by to mohlo byt pro OnceDiff 0 (press backspace or delete to remove)To je sice pravda, ale ani v originalnim OnceDifferentiable nebylo h_calls?
    ...
    return MultivariateOptimizationResults{typeof(method),Tx,typeof(x_abschange(state)),Tf,typeof(tr), Bool, typeof(stopped_by)}(method,
                                        ...
                                        f_calls(d),
                                        g_calls(d),
                                        0,#h_calls(d), #Tady ma byt h_calls ale to neexistuje ani v originalni funkci :DDDD TODO
                                        ls_success,
                                        options.time_limit,
                                        _time-t0,
                                        stopped_by,
...
function value_gradient_CUDA!(obj::D, x::CUDA.CuArray{T}) where {D,T}
    if x != obj.x_f && x != obj.x_df
        value_gradient_CUDA!!(obj, x)
    elseif x != obj.x_f
        value!!(obj, x)
    elseif x != obj.x_df
        gradient!!(obj, x)
